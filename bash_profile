...
# Virtualenv setup
alias venv='source venv/bin/activate 2> /dev/null || source venv3/bin/activate 2> /dev/null || source .venv/bin/activate 2> /dev/null || source .venv3/bin/activate'
...
export FZF_ANSI_OPTS="--color=bg+:$COLOR_DIM_BG"

# PIAS -- Play It Again Sam!
alias pias='eval $(cat ~/.dotfiles/pias/* | fzf --border-label="Select command to eval" --border=top --no-sort)'
